  ...
  for position, column := range table.Columns {
    ...
    for _, currentColumn := range current.Columns {
      if column.Name == currentColumn.Name {
        switch false {
        case column.FullType == currentColumn.FullType:
          fallthrough
        case column.Null == currentColumn.Null:
          fallthrough
        case column.Default == currentColumn.Default:
          fallthrough
        case column.Charset == currentColumn.Charset:
          fallthrough
        case column.Collation == currentColumn.Collation:
          fallthrough
  ...
  for _, index := range table.Indexes {
    ...
    for _, currentIndex := range current.Indexes {
      if index.Name == currentIndex.Name {
        ...
        case equals:
          fallthrough
        case index.Unique == currentIndex.Unique:
          fallthrough
        case index.Collation == currentIndex.Collation:
          fallthrough
        case index.Null == currentIndex.Null:
          fallthrough
  ...
  for _, fk := range table.ForeignKeys {
    ...
    for _, currentFk := range current.ForeignKeys {
      if fk.Name == currentFk.Name {
        ...
        case equals:
          fallthrough
        case db == currentFk.Schema:
          fallthrough
        case fk.Table == currentFk.Table:
          fallthrough
        case fk.OnUpdate == currentFk.OnUpdate:
          fallthrough
